...
$(function() {
	...
	$(".storeCategoryArea>li:nth-child(3) .storeSecondCategoryArea li:nth-child(2)").addClass("checkedStateSecondCategory");
})
</script>
</head>
<body>
				...
				<table border="1">
					...
					<c:forEach items="${orderCheckList}" var="orderCheck" >
						<tr data-s_id="${orderCheck.s_id}">
							...
							<td>
								<fmt:parseDate value="${orderCheck.o_date}" pattern="yyyy-MM-dd HH:mm:ss" var="parsedDate" />
								<fmt:formatDate value="${parsedDate}" pattern="yyyy/MM/dd" />
							</td> 
							<td>
								<fmt:parseDate value="${orderCheck.p_date}" pattern="yyyy-MM-dd HH:mm:ss" var="parsedDate" />
								<fmt:formatDate value="${parsedDate}" pattern="yyyy/MM/dd" />
							</td>
							<td>${orderCheck.p_sum}</td>
							<td>${orderCheck.p_ck}</td>
							<td>
								<fmt:parseDate value="${orderCheck.p_ck_date}" pattern="yyyy-MM-dd HH:mm:ss" var="parsedDate" />
								<fmt:formatDate value="${parsedDate}" pattern="yyyy/MM/dd" />
							</td>
    ...
    $(".check-delivery").on("click", function() {
        ...
        $.ajax({
            ...
            success: function(data) {
				...
				$(".storeMoreInfoModalWrap").fadeIn(200);
				$(".storeDeliveryModal").slideDown(200);
            },
            error: function() {
                alert("주문 세부 정보를 가져오는 중에 오류가 발생했습니다.");
            }
